 ...
 *  \version 1.0
 *
 *  \brief   Simple I2C LCD library to write text to a I2C lcd (PCF8574T adress 0x27) with the ATxmega256a3u (optional: trough a TCA9548A I2C switch).
 *
...
{
// ---- BY USE WITH THE TCA9548A I2C SWITCH USE THE FOLLOWED CODE UNTIL -- END OF CODE --
	char cmd_byte = 0X00;
	// In my example use only channel 2
	cmd_byte = (CMD_TCA9548A_OFF| CMD_TCA9548A_CH2);
		
	i2c_start(&TWIE, I2C_TCA9548A_ADDR, 0);
	i2c_write(&TWIE, cmd_byte);
	i2c_stop(&TWIE);
// ---- END OF CODE FOR THE TCA9548A I2C SWITCH ------------------------------------------
